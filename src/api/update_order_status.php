...
try {
    ...
    if (!isset($input['order_id']) || !isset($input['status'])) {
        sendValidationError(['order_id' => 'Order ID is required', 'status' => 'Status is required']);
    }
    ...
    // Validate order ID
    if ($order_id <= 0) {
        sendValidationError(['order_id' => 'Invalid order ID']);
    }
    
    // Validate status
    $allowed_statuses = ['Pending', 'Processing', 'Shipped', 'Delivered', 'Cancelled', 'Returned'];
    if (!in_array($new_status, $allowed_statuses)) {
        sendValidationError(['status' => 'Invalid status. Allowed: ' . implode(', ', $allowed_statuses)]);
    }
